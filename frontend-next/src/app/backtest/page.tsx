  ...
  return (
    <div className="h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 text-white flex flex-col overflow-hidden">
      ...
      <main ref={containerRef} className="flex-1 flex flex-col min-h-0">
        ...
        >
          <div className="h-full flex items-center justify-center text-white/40 border border-white/10 rounded-xl">
            {/* 차트 컴포넌트 placeholder - 다른 유닛에서 마이그레이션 예정 */}
            <p className="text-sm">차트 영역 (마이그레이션 예정)</p>
          </div>
        </div>
        ...
        <div style={{ flex: `${bottomPanelPercent}` }} className="min-h-0">
          <div className="h-full flex items-center justify-center text-white/40 border-t border-white/10">
            {/* 탭 패널 placeholder - 다른 유닛에서 마이그레이션 예정 */}
            <div className="text-center">
              <p className="text-sm mb-2">하단 탭 패널 (마이그레이션 예정)</p>
              {result && (
                <p className="text-xs text-green-400">
                  백테스트 결과: {result.totalReturn.toFixed(1)}% 수익률, {result.totalTrades}건 거래
                </p>
              )}
            </div>
          </div>

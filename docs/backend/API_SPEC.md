...
AI 프로바이더 오류는 응답 본문을 그대로 내려보내지 않습니다. 프로바이더의 401 본문에는
사용된 키가 마스킹된 형태로 들어 있어서 그대로 노출하면 서버 설정이 새어 나갑니다.
원문은 서버 로그에만 남기고, 클라이언트에는 아래 상태 코드와 고정 문구만 갑니다
(`services/ai_errors.py`).

| 상태 | 상황 | detail |
|------|------|--------|
| 400 | 프롬프트가 비었거나 1000자 초과, AI가 만든 식이 검증 실패 | 상황별 안내 문구 |
| 429 | IP rate limit 또는 프로바이더 사용량 한도 | 잠시 후 재시도 안내 |
| 502 | 프로바이더가 그 밖의 오류 반환, 응답 파싱 실패 | AI 서비스 오류 안내 |
| 503 | API 키 미설정·인증 실패, 프로바이더 연결 실패 | 관리자 문의 안내 |
| 504 | 프로바이더 응답 지연 | 재시도 안내 |

`/api/ai/generate-report`도 같은 규칙을 씁니다.
